...
# -- Plot the trajectory on UMAP with colors by cluster 
print( plot_cells( cds_sc10X_seurat,
            ...
            graph_label_size = 4,
            cell_size = 0.5)  + ggtitle( paste( "Trajectory with cluster colors for\n cluster resolution", CLUSTER_RESOLUTION)))
...
if( CLUSTER_RESOLUTION == 0.003 || CLUSTER_RESOLUTION == 0.001){
  
  # Load the KEGG pathways that will be used later
  ...
  vertex_degrees <- igraph::degree( cds_sc10X_seurat@principal_graph$UMAP)
  ...
  # Add to the pairs of vertex the custom ones
  ...
  for( pair_index in 1:ncol( pairs_vertex_degree_1)){
    ...
    cat( "\n \n")
    cat( "\n##### Identification of Gene differentially expressed along pseudotime")
    cat( "\n \n")
    
    # -- compute the genes that are differentially expressed along the pseudotime
    
    path_cds_sc10X_seurat = cds_sc10X_seurat[ , path_cells$cell.id]
    
    modulated_genes <- graph_test( path_cds_sc10X_seurat, neighbor_graph = "principal_graph", cores = 1, verbose = FALSE)
    modulated_genes = modulated_genes[ order( modulated_genes$q_value, modulated_genes$morans_I, decreasing = c( FALSE, TRUE)),]
    path_DEG_genes <- row.names( subset( modulated_genes, q_value <= 0.05))
    
    # -- Print the datatable of DEg along pseudotime
    DT::datatable( modulated_genes[ path_DEG_genes, ])
    
    # -- limit the list of DEG genes to top 50 for the heatmap
    if( length( path_DEG_genes) > 50){
      path_DEG_genes = path_DEG_genes[ 1:50]  
    }
    
    # -- compute the matrix of expression of genes differentially expresses along pseudotime
    pt.matrix <- exprs( path_cds_sc10X_seurat)[ match( path_DEG_genes, rownames( rowData( path_cds_sc10X_seurat))), order( pseudotime( path_cds_sc10X_seurat))]
    ...
    Heatmap(
      ...
      column_title = "Expression Z-score of top 50 DEG along pseudotime clustered by Ward method"
    )
    )
    
    cat( "\n \n")
    cat( "\n##### Expression along path of genes DE along pseudotime  {.tabset}")
    cat( "\n \n")
    
    # Provide the expression of pseudotime DEG genes
    ...
    # Change the "-" character to "." in gene names because of issues with "-" in ggplot
    names( path_cells_DEG_expression) = gsub( "-", ".", names( path_cells_DEG_expression))
    
    # For each monitored gene, look at its expression along the trajectory
    for( de_gene in path_DEG_genes){
      ...
      # Convert eventual "-" to "." in the gene name
      de_gene = gsub( "-", ".", de_gene)
      
      # Plot the computed statistics and expression of gene along path
      print( ggplot( data = path_cells_DEG_expression, aes_string(  x="pseudotime", y = de_gene)) + 
               geom_point( aes( color = pseudotime)) + geom_smooth( formula = 'y ~ x', method = "loess") +
               scale_colour_viridis_c( option = "plasma") +
               theme_minimal() + ggtitle( paste( "Expression of", de_gene, "along pseudotime for all cells"))
      )
      
      print( ggplot(  data = NULL, aes_string(  x="pseudotime", y = de_gene)) + 
               geom_point( data = path_cells_DEG_expression[ which( path_cells_DEG_expression$cell.id %in% WT_cells), ], color = SAMPLE_COLOR[[ SAMPLE_WT]]) +
               geom_smooth( data = path_cells_DEG_expression[ which( path_cells_DEG_expression$cell.id %in% WT_cells), ], formula = 'y ~ x', method = "loess", color = SAMPLE_COLOR[[ SAMPLE_WT]]) + 
               geom_point( data = path_cells_DEG_expression[ which( path_cells_DEG_expression$cell.id %in% KO_cells), ], color = SAMPLE_COLOR[[ SAMPLE_KO]]) +
               geom_smooth( data = path_cells_DEG_expression[ which( path_cells_DEG_expression$cell.id %in% KO_cells), ], formula = 'y ~ x', method = "loess", color = SAMPLE_COLOR[[ SAMPLE_KO]]) + 
               theme_minimal() + ggtitle( paste( "Expression of", de_gene, "along pseudotime for WT and KO cells"))
      )
    ...
    for( kegg_pathway in names( MONITORED_KEGG_PATHWAY_LIST)){
      ...
      # Provide the score of the KEGG Pathway
      kegg_score = data.frame( kegg.score = sc10x.seurat@meta.data[ path_cells$cell.id, paste0( kegg_pathway, "1")])
      kegg_score$cell.id = path_cells$cell.id
      path_kegg_score = merge( path_cells, kegg_score, by = "cell.id")
      path_kegg_score = path_kegg_score[ order( path_kegg_score$pseudotime, decreasing = FALSE), ]
      
      # Plot the scores of group of gene along path
      print( ggplot( data = path_kegg_score, aes(  x=pseudotime, y = kegg.score)) + 
               geom_point( aes( color = pseudotime)) + geom_smooth( formula = 'y ~ x', method = "loess") +
               scale_colour_viridis_c( option = "plasma") +
               theme_minimal() + ggtitle( paste( "Score of", kegg_pathway, "along pseudotime for all cells"))
      )
      
      print( ggplot(  data = NULL, aes(  x=pseudotime, y = kegg.score)) + 
               geom_point( data = path_kegg_score[ which( path_kegg_score$cell.id %in% WT_cells), ], color = SAMPLE_COLOR[[ SAMPLE_WT]]) +
               geom_smooth( data = path_kegg_score[ which( path_kegg_score$cell.id %in% WT_cells), ], formula = 'y ~ x', method = "loess", color = SAMPLE_COLOR[[ SAMPLE_WT]]) + 
               geom_point( data = path_kegg_score[ which( path_kegg_score$cell.id %in% KO_cells), ], color = SAMPLE_COLOR[[ SAMPLE_KO]]) +
               geom_smooth( data = path_kegg_score[ which( path_kegg_score$cell.id %in% KO_cells), ], formula = 'y ~ x', method = "loess", color = SAMPLE_COLOR[[ SAMPLE_KO]]) + 
               theme_minimal() + ggtitle( paste( "Score of", kegg_pathway, "along pseudotime for WT and KO cells"))
      )
