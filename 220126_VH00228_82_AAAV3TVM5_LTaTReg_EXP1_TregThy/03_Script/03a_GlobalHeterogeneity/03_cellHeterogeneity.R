...
## Do it in pure GGplot
print( 
  DimPlot( sc10x, group.by = "seurat_clusters")  + ggtitle( 'UMAP of All Cells')
)



# If both conditions are present, plot both condition on the same UMAP but on different plots
if( SAMPLE_NAME == SAMPLE_ALL){
  
  # Identify the cells of each condition  
  Idents( sc10x) = "HTO_classification"
  WT_cells_names = names( Idents( sc10x))[ which( Idents( sc10x) == SAMPLE_WT)]
  Idents( sc10x) = "seurat_clusters"
  
  # Create the table to plot for each condition
  dimReducData_WT = dimReducData[ WT_cells_names,]
  
  #Plot the WT cells UMAP
  plotDimReduc_WT = plot_ly(dimReducData_WT,
                            x = as.formula( paste( "~", colnames( dimReducData_WT)[1])),
                            y = as.formula( paste( "~", colnames( dimReducData_WT)[2])),
                            #                       name = ~paste("Cluster", Cluster),
                            color = ~Cluster,
                            colors = clustersColor,
                            height = 800) %>%
    add_trace( type = "scattergl",
               ...
               marker = list( size = 5,
                              opacity = 0.6)) %>%
    # Compute median of coordinates for each group and use it as new data for Cluster text annotations (type='scatter', mode='text')
    add_trace( data = as.data.frame( do.call( rbind, by( dimReducData_WT, dimReducData_WT[["Cluster"]], function(x){ return( data.frame( lapply( x[1:2], median), "Cluster" = x[1, "Cluster"]));}))),
               x = as.formula( paste( "~", colnames( dimReducData_WT)[1])),
               y = as.formula( paste( "~", colnames( dimReducData_WT)[2])),
               type = "scattergl",
               mode = "text",
  ...
  #browsable(
  # div( style = paste("display: flex; align-items: center; justify-content: center;", if(.SHOWFLEXBORDERS) "border-style: solid; border-color: red;"),
  #      div( plotDimReduc_WT_ggplot, style = paste("flex : 0 0 800px;", if(.SHOWFLEXBORDERS) "border-style: solid; border-color: blue;")))
  #)
  
  ## Do it in pure GGplot
  Idents( sc10x) = "HTO_classification"
  print(
    DimPlot( sc10x, cells = names( Idents( sc10x))[ which( Idents( sc10x) == SAMPLE_WT)], group.by = "seurat_clusters") + ggtitle( 'UMAP of WT Cells')
  )
  Idents( sc10x) = "seurat_clusters"
}
...
if( SAMPLE_NAME == SAMPLE_ALL){
  ...
  # Create the table to plot for each condition
  dimReducData_KO = dimReducData[ KO_cells_names,]
  
  #Plot the KO cells UMAP
  plotDimReduc_KO = plot_ly(dimReducData_KO,
                            x = as.formula( paste( "~", colnames( dimReducData_KO)[1])),
                            y = as.formula( paste( "~", colnames( dimReducData_KO)[2])),
                            #                       name = ~paste("Cluster", Cluster),
                            color = ~Cluster,
                            colors = clustersColor,
                            height = 800) %>%
    add_trace( type = "scattergl",
               ...
               marker = list( size = 5,
                              opacity = 0.6)) %>%
    # Compute median of coordinates for each group and use it as new data for Cluster text annotations (type='scatter', mode='text')
    add_trace( data = as.data.frame( do.call( rbind, by( dimReducData_KO, dimReducData_KO[["Cluster"]], function(x){ return( data.frame( lapply( x[1:2], median), "Cluster" = x[1, "Cluster"]));}))),
               x = as.formula( paste( "~", colnames( dimReducData_KO)[1])),
               y = as.formula( paste( "~", colnames( dimReducData_KO)[2])),
               type = "scattergl",
               mode = "text",
  ...
  #browsable(
  #div( style = paste("display: flex; align-items: center; justify-content: center;", if(.SHOWFLEXBORDERS) "border-style: solid; border-color: red;"),
  #     div( plotDimReduc_KO_ggplot, style = paste("flex : 0 0 800px;", if(.SHOWFLEXBORDERS) "border-style: solid; border-color: blue;")))
  #)
  
  ## Do it in pure GGplot
  Idents( sc10x) = "HTO_classification"
  print( 
    DimPlot( sc10x, cells = names( Idents( sc10x))[ which( Idents( sc10x) == SAMPLE_KO)], group.by = "seurat_clusters") + ggtitle( 'UMAP of KO Cells')
  )
  Idents( sc10x) = "seurat_clusters"
...
pheatmap( expMat,
          ...
          show_colnames = FALSE,
          fontsize_row = 5,
          main = "Normalized expression by cell\nof top markers");
...
```{r heterogeneity_monitoredGenes_heatmapInteractive, echo = FALSE}
    ...
    heatmapPlot = iheatmap( expMat,
                            ...
                            tooltip = setup_tooltip_options(row = TRUE,
                                                            ...
                                                            prepend_value = ""),
                            row_annotation = rowsAnnot,
                            row_annotation_colors = if(nlevels( rowsAnnot[["Monitored"]]) <= 1) NULL else list( Monitored = rainbow( nlevels( rowsAnnot[["Monitored"]]), s = 0.8)),
                            col_annotation = colsAnnot,
                            col_annotation_colors = list( Cluster = clustersColor),
                            layout = list(width = 900, height = 900));
