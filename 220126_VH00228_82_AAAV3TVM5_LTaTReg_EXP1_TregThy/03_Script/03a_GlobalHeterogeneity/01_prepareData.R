...
hs_stress_genes_df = merge( hs_stress_genes_df, converted_genes_df, by.x = "gene_symbol", by.y="HGNC.symbol")
hs_stress_mouse_genes_set = na.exclude( unique( hs_stress_genes_df$MGI.symbol))
...
MODULES_GENES[[ "HeatShock"]] = unique( hs_stress_genes_df[ , "MGI.symbol"])
MODULES_GENES[[ "HeatShock_Top40"]] = unique( hs_stress_genes_df[ 1:40, "MGI.symbol"])
...
## @knitr filterData_bySample

# If only one sample is selected, get the cells only from that sample
if( SAMPLE_NAME != SAMPLE_ALL){
  Idents( sc10x) = "HTO_classification"
  selected_cells = names( Idents( sc10x))[ which( Idents( sc10x) == SAMPLE_NAME)]
  sc10x = subset( sc10x, cells = selected_cells)
}
...
# sce <- SingleCellExperiment( list( counts=sc10x@assays$RNA@counts))
# 
# ## Calculate doublet ratio ###
# doublet_ratio <- ncol(sce)/1000*0.008
# 
# ### Calculate Singlets and Doublets ###
# sce <- scDblFinder(sce, dbr=0.06)
# 
...
# 
# table( sc10x$HTO_classification.global, sc10x$scDblFinder)
