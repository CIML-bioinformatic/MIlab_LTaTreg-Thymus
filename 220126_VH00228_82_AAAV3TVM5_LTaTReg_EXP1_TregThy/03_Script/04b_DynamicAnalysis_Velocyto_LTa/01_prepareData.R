...
# Get the UMAP and Sample of origin from the Seurat object
umap_embedding = as.data.frame( sc10x.seurat@reductions$umap@cell.embeddings)
Idents( sc10x.seurat) = "HTO_classification"
umap_embedding$Sample = Idents( sc10x.seurat)[ row.names( umap_embedding)]

# Take WT cells only
umap_embedding = umap_embedding[ row.names( umap_embedding)[ which( umap_embedding$Sample == "Lta-TregThy")], ]

# Convert the names of the cells (they are different in the Seurat Object and in the loom object)
# RUN1_THY1_ACGGGTCAGATGATTG-1 becomes mm10:ACGGGTCAGATGATTGx
...
# Get the WT cells names from the pagoda cells names
LTa_cells = row.names( umap_embedding)
