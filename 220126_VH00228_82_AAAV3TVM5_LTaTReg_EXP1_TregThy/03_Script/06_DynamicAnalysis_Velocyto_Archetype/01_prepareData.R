...
sc10x.seurat = readRDS( SEURAT_OBJECT_ARCHETYPAL_ANALYSIS)
...
umap_embedding$BestArchetype = Idents( sc10x.seurat)[ row.names( umap_embedding)]

# Convert the names of the cells (they are different in the Seurat Object and in the loom object)
# RUN1_SPL1_ACGGGTCAGATGATTG-1 -> mm10:ACGGGTCAGATGATTGx
row.names( umap_embedding) = gsub( "RUN1_SPL1_", "mm10:", row.names( umap_embedding))
...
# Get the archetype as clusters
archetype_cell_set = umap_embedding[ names( r$clusters$PCA[[ "multilevel"]]), "BestArchetype"]
names( archetype_cell_set) = names( r$clusters$PCA[[ "multilevel"]])
r$clusters$PCA[[ "Archetype"]] = archetype_cell_set
r$clusters$PCA[[ "Archetype"]][ which( is.na( r$clusters$PCA[[ "Archetype"]]))] = 1
...
kable( cluster_df, caption="Distribution of cells along clusters", rownames=FALSE)

# Compute differentially expressed genes
#r$getDifferentialGenes(type='PCA', verbose=T, clusterType='multilevel')
r$getDifferentialGenes(type='PCA', verbose=T, clusterType='Archetype')
de = data.frame()
